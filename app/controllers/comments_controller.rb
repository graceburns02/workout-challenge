class CommentsController < ApplicationController
  ...
  def create
    ...
    the_comment.workout_id = params.fetch("query_workout_id")

    if the_comment.valid?
      the_comment.save
      redirect_to("/workouts", { :notice => "Comment created successfully." })
    else
      redirect_to("/workouts", { :alert => the_comment.errors.full_messages.to_sentence })
    end
  end
  ...
  def update
    ...
    if the_comment.valid?
      the_comment.save
      redirect_to("/workouts/", { :notice => "Comment updated successfully."} )
    else
      redirect_to("/workouts/", { :alert => the_comment.errors.full_messages.to_sentence })
    end
  end
  ...
  def destroy
    ...
    redirect_to("/workouts", { :notice => "Comment deleted successfully."} )
  end
end
